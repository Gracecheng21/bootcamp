// ! During complie-time, the way to ensure a object comply with a contract (with method)
...
public class Book implements Comparable<Book> { // ! method, input: String, output: int
  ...
  public int compareTo(Book book) { // ! The definition of ordering of Book
    // return this.name.compareTo(book.getName()); // ! asscending order
    return book.getName().compareTo(this.name); // ! desending order
  }
  ...
  // 如果想出book name 唔係用 .getName() -> 要自已override toString()
  @Override
  ...
  public static void main(String[] args) {
    System.out.println("abc".compareTo("ccd")); // -2 c比a大2格，a不是大過c，所以-2
    System.out.println("abc".compareTo("abc")); // 0 
    System.out.println("bbc".compareTo("abc")); // 1
    System.out.println("bbc".compareTo("bcc")); // -1

    int result = compareBook(new Book("DEF"), new Book("ABC"));
