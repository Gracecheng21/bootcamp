...
public class StreamExercise {
  public static void main(String[] args) {
    ...
    .filter(n -> n % 2 == 0) // 
    .map(n -> n * n) // map 係用嚟「轉換」stream 裏面嘅元素
    .sorted((n1 , n2) -> n1 > n2 ? -1 : 1)
    .collect(Collectors.toList()); // collect集 ， （Collectors.toList() 轉出 new List)
    System.out.println(result); // [100, 64, 36, 16, 4]

    // .filter(n -> n % 2 == 0) → 保留偶數。
    // .map(n -> n * n) → 將每個數字平方。
    // .sorted(Comparator.reverseOrder()) → 用 Comparator 對 List 做降序排列, Comparator(比較<>入面所有大細，natrualOrder/reverseOrder)
    // .collect(Collectors.toList()) → 將結果變成 List。
    ...
    List<String> nameListA = names.stream()
    .filter(n -> n.startsWith("A"))
    .collect(Collectors.toList());
    System.out.println(nameListA); // [Alice, Annie, Alex]
    ...
    // Output: 4
    long wordsLength2 = words2.stream()
    .filter(w -> w.length() > 3)
    .count();
    System.out.println(wordsLength2); // 4
    ...
    // Output: [15, 20]
    Set<Integer> q6result = numbers3.stream() //
    .filter(e -> e > 10) //
    .collect(Collectors.toSet());
    System.out.println(q6result); // [20, 15]
    ...
    employees.add(new Employee("Doe", 44000));
    //List<String> q8result = employees.stream() //
